...
func opSload(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if interpreter.evm.OCCLocalManager == nil {
		...
		//先检查localstate,对应的stateobject的dirtykey是否存在,read操作必须要读到最新的状态,优先级1.本地已经被修改/读取过的状态，2.全局的最新状态
		exit := interpreter.evm.StateDB.CheckState(callContext.contract.Address(), common.BigToHash(loc))	//先检查local是否存在dirty的值，即读取的内容是否被本Tx修改过
		if !exit {
			//这里很容易引起冲突，缺少安全的锁机制
			//interpreter.evm.OCCLocalManager.OCCGlobalManagerPointer.Mu.Lock()
			readRecord = interpreter.evm.OCCLocalManager.OCCGlobalManagerPointer.GetRecordFromGlobal(callContext.contract.Address(), common.BigToHash(loc))		//先拿到record防止脏读
			//val = interpreter.evm.GlobalStateDB.GetState(callContext.contract.Address(), common.BigToHash(loc))		//如果local没有修改过，去global读
			//类似2PL的优化
			val = interpreter.evm.GlobalStateDB.GetState(callContext.contract.Address(), common.BigToHash(loc))		//如果local没有修改过，去global读
			//interpreter.evm.OCCLocalManager.OCCGlobalManagerPointer.Mu.Unlock()
		} else {
			//interpreter.evm.OCCLocalManager.OCCGlobalManagerPointer.Mu.Lock()
			readRecord = interpreter.evm.OCCLocalManager.OCCGlobalManagerPointer.GetRecordFromGlobal(callContext.contract.Address(), common.BigToHash(loc))
			val = interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(loc))			//如果local修改过，去local读dirty的值
			//interpreter.evm.OCCLocalManager.OCCGlobalManagerPointer.Mu.Unlock()
		}

		/*fmt.Println("opSload")
		fmt.Println(common.BigToHash(loc))
		fmt.Println(*pc)*/

		//把读写记录到rwset中
		temp := occmanager.BytesCombine(callContext.contract.Address().Bytes(), common.BigToHash(loc).Bytes())
		interpreter.evm.OCCLocalManager.ReadSet[md5.Sum(temp)] = readRecord
...
func opSstore(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	if interpreter.evm.OCCLocalManager == nil {
		...
		//把读写记录到rwset中
		writeRecord := interpreter.evm.OCCLocalManager.AddWriteRecord(callContext.contract.Address(), loc)
		if writeRecord != nil {
			temp := occmanager.BytesCombine(callContext.contract.Address().Bytes(), loc.Bytes())
			interpreter.evm.OCCLocalManager.WriteSet[md5.Sum(temp)] = writeRecord
		}
