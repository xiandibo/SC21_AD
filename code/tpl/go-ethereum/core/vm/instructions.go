...
func opSload(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if interpreter.evm.TPLLocalManager == nil {
		var val common.Hash
		//var readRecord *occmanager.Record

		loc := callContext.stack.peek()

		exit := interpreter.evm.StateDB.CheckState(callContext.contract.Address(), common.BigToHash(loc))	//先检查local是否存在dirty的值，即读取的内容是否被本Tx修改过
		if !exit {
			//会使得RPC调用空指针异常
			if interpreter.evm.GlobalStateDB != nil {
				val = interpreter.evm.GlobalStateDB.GetState(callContext.contract.Address(), common.BigToHash(loc))
			} else {
				val = interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(loc))
			}
		} else {
		...
		count := 0
		var result bool
		var record *twoplmanager.Record
		const (
			readLock = 2
			//writelock = 3
		)

		//result, record := interpreter.evm.TwoPLGlobalManger.ApplyForLock(2, callContext.contract.Address(), common.BigToHash(loc))	//申请读锁

		//首先申请锁
		for {

			//首先判断是否已经申请过锁
			if interpreter.evm.TPLLocalManager.IsExistReadLock(callContext.contract.Address(), common.BigToHash(loc)) || interpreter.evm.TPLLocalManager.IsExistWriteLock(callContext.contract.Address(), common.BigToHash(loc)) {
				result = true
				record = interpreter.evm.TwoPLGlobalManger.GetRecord(callContext.contract.Address(), common.BigToHash(loc))
				break //本事务已经申请过此锁，跳过申请锁阶段
			}
			//在本事务没有成功申请到读锁或写锁的情况下，才去申请读锁
			result, record = interpreter.evm.TwoPLGlobalManger.ApplyForLock(readLock, callContext.contract.Address(), common.BigToHash(loc)) //申请read锁
			count++
			//fmt.Println(count)
			if result == true {
				break
				...
				//temp := rand.Intn(100)
				//time.Sleep(time.Microsecond * time.Duration(temp))
				time.Sleep(time.Millisecond * 5)
			}
			if count == 2 { //如果2ms内申请不到锁，视为死锁，中止交易并回滚
				//超时，可能发生死锁
				interpreter.evm.DeadLockAbortFlag = true
				//fmt.Println("opSload Rollback")
				//也可以直接在这里回滚
				for _, record := range interpreter.evm.TPLLocalManager.WriteSet {
					interpreter.evm.GlobalStateDB.SetStateFor2PL(record.Address, record.Key, record.PreValue)
				}
		...
		if result == true {
			...
			interpreter.evm.TPLLocalManager.AddLockedRecord(callContext.contract.Address(), common.BigToHash(loc), record)
			interpreter.evm.TPLLocalManager.AddReadSet(callContext.contract.Address(), common.BigToHash(loc), record)

			val := interpreter.evm.GlobalStateDB.GetStateFor2PL(callContext.contract.Address(), common.BigToHash(loc))

			//record.SetPreValue(val)
		...
		loc := callContext.stack.peek()
		//_, record := interpreter.evm.TwoPLGlobalManger.ApplyForLock(2, callContext.contract.Address(), common.BigToHash(loc))	//申请读锁
		//interpreter.evm.TPLLocalManager.AddLockedRecord(callContext.contract.Address(), common.BigToHash(loc), record)
		//interpreter.evm.TPLLocalManager.AddReadSet(callContext.contract.Address(), common.BigToHash(loc), record)

		//interpreter.evm.TwoPLGlobalManger.GetRecord(callContext.contract.Address(), common.BigToHash(loc))
...
func opSstore(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if interpreter.evm.TPLLocalManager == nil {
		...
		//result, record = interpreter.evm.TwoPLGlobalManger.ApplyForLock(3, callContext.contract.Address(), loc)	//申请写锁

		for {

			//首先判断是否已经申请过写锁
			if interpreter.evm.TPLLocalManager.IsExistWriteLock(callContext.contract.Address(), loc) {
				result = true
				record = interpreter.evm.TwoPLGlobalManger.GetRecord(callContext.contract.Address(), loc)
				break //本事务已经申请过此锁，跳过申请锁阶段
			}
			//如果已经持有读锁，尝试更新到写锁
			if interpreter.evm.TPLLocalManager.IsExistReadLock(callContext.contract.Address(), loc) {
				result, record = interpreter.evm.TwoPLGlobalManger.UpdateForReadLock(callContext.contract.Address(), loc)
			} else {
				//如果没有持有读锁，直接申请写锁
				result, record = interpreter.evm.TwoPLGlobalManger.ApplyForLock(writelock, callContext.contract.Address(), loc) //申请写锁
			}
			count++
			if result == true {
				break
		...
		if result == true {
			...
			interpreter.evm.TPLLocalManager.AddLockedRecord(callContext.contract.Address(), loc, record)
			interpreter.evm.TPLLocalManager.AddWriteSet(callContext.contract.Address(), loc, record)

			//还需解决并发访问StateDB的问题
			tempPreValue := interpreter.evm.GlobalStateDB.GetStateFor2PL(callContext.contract.Address(), loc)
			record.SetPreValue(tempPreValue)
			interpreter.evm.GlobalStateDB.SetStateFor2PL(callContext.contract.Address(), loc, common.BigToHash(val))
		...
		loc := common.BigToHash(callContext.stack.pop())
		val := callContext.stack.pop()
		//_, record := interpreter.evm.TwoPLGlobalManger.ApplyForLock(3, callContext.contract.Address(), loc)	//申请写锁
		//interpreter.evm.TPLLocalManager.AddLockedRecord(callContext.contract.Address(), loc, record)
		//interpreter.evm.TPLLocalManager.AddWriteSet(callContext.contract.Address(), loc, record)

		//interpreter.evm.TwoPLGlobalManger.GetRecord(callContext.contract.Address(), loc)

		interpreter.evm.GlobalStateDB.SetState(callContext.contract.Address(), loc, common.BigToHash(val))
