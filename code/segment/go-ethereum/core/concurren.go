...
func ConcurValidate(TxSet types.Transactions, coinbase *common.Address, statedb *state.StateDB, cfg vm.Config, config *params.ChainConfig, bc *BlockChain, parent *types.Header, header *types.Header, usedGas *uint64) (types.Receipts, error) {
	...
	for _, v := range SeralizeTxSet {
		receipt, _ := ApplyCTxForMainTx(config, bc, coinbase, gasP, statedb, header, v, v.MainTx.UsedGas, cfg)
...
func ApplyCTxForCallTx(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, ctx *ConcurrentTransaction, usedGas *uint64, cfg vm.Config, CallType uint) ([]byte, error) {
	tx := ctx.Transaction
...
//执行MainTx
func ApplyCTxForMainTx(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, ctx *ConcurrentTransaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, error) {
	...
	if ctx.MainTx.EVMState.PC != 0 {
		...
		*usedGas += BeforeGas - Gas
		...
		if EVMSt.Halts == true {
			...
			receipt.TransactionIndex = ctx.TxIndex + 1
