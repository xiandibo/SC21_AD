...
func (s *StateDB) SetRe(value int) {
	s.txIndex = value
}
func (s *StateDB) GetRe() int {
	return s.txIndex
}
...
func (s *StateDB) FinaliseForThreads(deleteEmptyObjects bool) {
	for addr := range s.journal.dirties {
...
func (s *StateDB) AddStateToWorker(t *StateDB) {
	for addr := range t.stateObjectsPending {
		if  _, ok := s.stateObjectsPending[addr]; ok {
			//temp := new(big.Int).Add(s.stateObjects[addr].data.Balance, new(big.Int).SetUint64(uint64(t.journal.dirties[addr])))
			//s.stateObjects[addr].data.Balance = temp
			temp := s.stateObjects[addr].data.Nonce + uint64(t.journal.dirties[addr])
			s.stateObjects[addr].data.Nonce = temp
			//s.stateObjects[addr].data.Balance += big.NewInt(t.journal.dirties[addr])
			//s.stateObjects[addr].data.Balance
		} else {
			s.stateObjects[addr] = t.stateObjects[addr]
			s.stateObjectsPending[addr] = t.stateObjectsPending[addr]
			s.stateObjectsDirty[addr] = t.stateObjectsDirty[addr]
		}
	}
