...
func opReturnDataCopy(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	//fmt.Println(uint64(len(interpreter.returnData)))
	//fmt.Println(end.Uint64())
...
func opCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if interpreter.evm.CallReturnFlag == true {
		interpreter.evm.CallReturnFlag = false
		callContext.stack.push(interpreter.intPool.get().SetUint64(1))

		callContext.contract.Gas += interpreter.evm.ReturnGas

		return interpreter.evm.ReturnData, nil
	}
	...
	//如果没有错或回滚，就i把返回信息写在内存
	//但是使用REMIX调试这一步没有生效，而是通过返回ret让调用函数
	...
	//保存当前执行状态
	interpreter.evm.Memory = callContext.memory
	interpreter.evm.Stack = callContext.stack
	interpreter.evm.Rstack = callContext.rstack
	interpreter.evm.Contract = callContext.contract

	interpreter.evm.ToAddr = toAddr
	interpreter.evm.Args = args
	interpreter.evm.Gas = gas
	//val := new(big.Int)
	//*val =
	interpreter.evm.Value = *value
	interpreter.evm.PendingFlag = true

	interpreter.evm.CallType = 1
	interpreter.evm.CallReturnFlag = false
...
func opStaticCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	if interpreter.evm.CallReturnFlag == true {
		...
		/*if interpreter.evm.Err == nil || interpreter.evm.Err == ErrExecutionReverted {
			callContext.memory.Set(retOffset.Uint64(), retSize.Uint64(), ret)
		}*/
