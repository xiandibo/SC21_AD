...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	//如果不是第一次进入执行字节码阶段，则恢复其执行状态
	//如果是CallTx执行，不能恢复EVM状态，要使用新的
	if in.evm.PC != 0 && in.evm.MainOrCall == true {
		mem         = in.evm.Memory
		stack       = in.evm.Stack
		returns     = in.evm.Rstack

		pc = in.evm.PC
		callContext.memory = in.evm.Memory
		callContext.stack = in.evm.Stack
		callContext.rstack = in.evm.Rstack
		callContext.contract = in.evm.Contract
		//恢复调用结果
		//in.returnData = in.evm.ReturnData
		...
		/*if in.evm.PendingFlag == true {
			fmt.Printf("PendingFlag\n")
			in.evm.PC = pc
			return nil, nil
		}*/
		...
		case operation.reverts:
			//fmt.Printf("%+v\n", pc)
			//fmt.Printf("ErrExecutionReverted\n")
